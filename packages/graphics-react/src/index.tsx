...
'.ips-anim-fade{animation:ipsFade var(--dur,.5s) ease both}.ips-anim-slide,.ips-anim-slide-left{animation:ipsSlide var(--dur,.5s) cubic-bezier(.16,.75,.2,1) both}.ips-anim-slide-right{animation:ipsSlideRight var(--dur,.5s) cubic-bezier(.16,.75,.2,1) both}.ips-anim-slide-up{animation:ipsSlideUp var(--dur,.5s) cubic-bezier(.16,.75,.2,1) both}.ips-anim-slide-down{animation:ipsSlideDown var(--dur,.5s) cubic-bezier(.16,.75,.2,1) both}.ips-anim-broadcast-wipe,.ips-anim-wipe{animation:ipsWipe var(--dur,.5s) cubic-bezier(.16,.8,.2,1) both}.ips-anim-impact{animation:ipsImpact var(--dur,.5s) cubic-bezier(.08,.9,.18,1) both}.ips-anim-scale-pop{animation:ipsScalePop var(--dur,.5s) cubic-bezier(.12,.8,.18,1) both}.ips-anim-mask-reveal{animation:ipsMaskReveal var(--dur,.5s) cubic-bezier(.16,.8,.2,1) both}.ips-anim-light-sweep{animation:ipsFade var(--dur,.5s) ease both}.ips-anim-flash{animation:ipsFlash .32s ease both}.ips-anim-ball-impact{animation:ipsFade .1s linear both}.ips-anim-flip{animation:ipsFlip var(--dur,.5s) ease both}.ips-anim-pulse{animation:ipsScalePop var(--dur,.5s) ease both}.ips-exit-fade{animation:ipsFadeOut var(--exit,.32s) ease both!important}.ips-exit-slide,.ips-exit-slide-left{animation:ipsSlideOut var(--exit,.32s) ease both!important}.ips-exit-slide-right{animation:ipsSlideRightOut var(--exit,.32s) ease both!important}.ips-exit-wipe,.ips-exit-broadcast-wipe{animation:ipsWipeOut var(--exit,.32s) ease both!important}.ips-exit-scale-pop{animation:ipsScaleOut var(--exit,.32s) ease both!important}.ips-exit-flip{animation:ipsFlipOut var(--exit,.32s) ease both!important}',
'@keyframes ipsFade{from{opacity:0}to{opacity:1}}@keyframes ipsFadeOut{from{opacity:var(--op,1)}to{opacity:0}}@keyframes ipsSlide{from{opacity:0;transform:translate3d(70px,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}to{opacity:var(--op,1);transform:translate3d(0,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}}@keyframes ipsSlideRight{from{opacity:0;transform:translate3d(-70px,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}to{opacity:var(--op,1);transform:translate3d(0,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}}@keyframes ipsSlideUp{from{opacity:0;transform:translate3d(0,70px,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}to{opacity:var(--op,1);transform:translate3d(0,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}}@keyframes ipsSlideDown{from{opacity:0;transform:translate3d(0,-70px,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}to{opacity:var(--op,1);transform:translate3d(0,0,0) scale(var(--sx,1),var(--sy,1)) rotate(var(--rot,0deg))}}@keyframes ipsSlideOut{from{opacity:var(--op,1)}to{opacity:0;transform:translate3d(-60px,0,0)}}@keyframes ipsSlideRightOut{from{opacity:var(--op,1)}to{opacity:0;transform:translate3d(60px,0,0)}}',
'@keyframes ipsWipe{from{opacity:0;clip-path:inset(0 100% 0 0)}to{opacity:var(--op,1);clip-path:inset(0 0 0 0)}}@keyframes ipsWipeOut{from{opacity:var(--op,1);clip-path:inset(0 0 0 0)}to{opacity:0;clip-path:inset(0 0 0 100%)}}@keyframes ipsScaleOut{from{opacity:var(--op,1);transform:scale(1)}to{opacity:0;transform:scale(1.1)}}@keyframes ipsFlip{from{opacity:0;transform:perspective(900px) rotateX(70deg)}to{opacity:var(--op,1);transform:perspective(900px) rotateX(0)}}@keyframes ipsFlipOut{from{opacity:var(--op,1);transform:perspective(900px) rotateX(0)}to{opacity:0;transform:perspective(900px) rotateX(-70deg)}}@keyframes ipsImpact{0%{opacity:0;transform:scale(.7) rotate(var(--rot,0deg));filter:blur(12px)}65%{opacity:1;transform:scale(1.045) rotate(var(--rot,0deg));filter:blur(0)}100%{opacity:var(--op,1);transform:scale(1) rotate(var(--rot,0deg))}}',
...
function ElementView({element,exiting=false}:{element:ResolvedElement;exiting?:boolean}){
  ...
  const enter=preset(element.animation?.enterPreset);\n  const exit=exiting&&element.animation?.exitPreset?' ips-exit-'+element.animation.exitPreset.toLowerCase().replaceAll('_','-'):'';\n  const cls='ips-el'+enter+exit;
...
export function SceneCanvas({document,data,scale=1,className='',showSafeArea=false,exiting=false}:{document:SceneDocument|unknown;data:unknown;scale?:number;className?:string;showSafeArea?:boolean;exiting?:boolean}){
  const model=useMemo(()=>createRenderModel(document,data),[document,data]);
  return <div className={'ips-scene '+className} style={{transform:'scale('+scale+')'}}><style>{CSS}</style>{showSafeArea&&<div style={{position:'absolute',left:model.document.safeArea.left,top:model.document.safeArea.top,right:model.document.safeArea.right,bottom:model.document.safeArea.bottom,border:'1px dashed rgba(25,209,143,.35)',zIndex:9999,pointerEvents:'none'}}/>}{model.elements.map(el=><ElementView key={el.id} element={el} exiting={exiting}/>)}</div>;
}
