...
export default async function RegistrationQueuePage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const {data,error}=await supabase.rpc('ips_registration_queue');
  const players=data?.players??[];
  const teams=data?.teams??[];
  const transfers=data?.transfers??[];
  const total=players.length+teams.length+transfers.length;
  ...
  return <main className="shell sports-shell">
    ...
    <section className="registration-queue-kpis">
      ...
      <article><span>TRANSFERS</span><strong>{transfers.length}</strong><small>release / final approval</small></article>
    </section>
