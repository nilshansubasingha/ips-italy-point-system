  ...
  try{
    ...
    go(`/manage/teams/${teamId}/players/add`,'ok',
      status==='REQUESTED_TRANSFER'
        ?'Transfer request sent. The player or their current Team must accept before the roster changes.'
        :status==='REQUESTED_FREE'
          ?'Join request sent. The player must accept before they are added to the roster.'
          :'Player request submitted.'
    );
