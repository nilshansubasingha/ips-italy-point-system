...
import { createTournament,createCatalogSeason,createCatalogRuleset } from '../actions';
...
export default async function NewTournamentPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [seasonsRes,rulesRes,venuesRes]=await Promise.all([
 ...
 return <main className="shell sports-shell"><SiteHeader/><ManagementNav account={account} active="tournaments"/>
   ...
   <form action={createTournament} className="tournament-workspace"><input type="hidden" name="return_to" value="/manage/tournaments/new"/>
     <div className="tournament-form-canvas"><TournamentCreatorFields allowedCities={allowedCities} seasons={(seasonsRes.data??[]) as any[]} rulesets={(rulesRes.data??[]) as any[]} venues={(venuesRes.data??[]) as any[]}/></div>
   ...
   {canCatalog&&<details className="management-surface catalog-workspace"><summary>Catalogue setup · add a season or reusable ruleset</summary><div className="catalog-light-grid">
