...
export default async function ManageTeamsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 <section className="management-surface"><div className="surface-head"><div><span className="eyebrow">ALL TEAMS</span><h2>Roster operations</h2></div></div><div className="team-table">{(teams??[]).map((t:any)=><Link href={`/manage/teams/${t.id}`} key={t.id} className="team-table-row"><Crest name={t.name} imageUrl={t.logo_url}/><div><h3>{t.name}</h3><p>{(t.club as any)?.name} · {(t.club as any)?.city?.name??'Italy'}</p></div><span className="team-category">{t.category}</span><strong>{counts.get(t.id)||0}<small> players</small></strong><i>→</i></Link>)}</div></section><SiteFooter/></main>;
}
