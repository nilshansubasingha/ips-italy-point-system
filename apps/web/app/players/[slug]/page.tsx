...
import { getPlayerBySlug } from '@ips/data';
...
export default async function PlayerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="shell sports-shell"><SiteHeader />
    <section className="entity-hero premium-entity-hero player-entity-hero premium-player-hero">
      ...
      <div className="entity-main"><div className="entity-kicker"><code>{player.ips_code}</code><span>{player.city?.name ?? 'Italy'}</span></div><h1>{player.display_name}</h1><p>{player.primary_role ?? 'Player'}{player.currentTeam ? ` · Current team: ${player.currentTeam.name}` : ' · Currently unattached'}</p><div className="entity-actions"><Link href="/rankings">Ranking profile →</Link>{player.currentClub && <Link href={`/clubs/${player.currentClub.slug}`}>Club profile →</Link>}</div></div>
    ...
    <section className="sports-section compact-section profile-columns premium-profile-columns">
      <div><div className="sports-section-head"><div><span className="eyebrow">CURRENT CLUB</span><h2>Official membership</h2></div></div>{player.currentClub ? <Link href={`/clubs/${player.currentClub.slug}`} className="current-club-card premium-current-club"><Crest name={player.currentClub.name} imageUrl={player.currentClub.logo_url} large/><div><span>{player.city?.name ?? 'Italy'}</span><h3>{player.currentClub.name}</h3><p>{player.currentTeam?.name ?? 'Team'}{player.shirtNumber ? ` · #${player.shirtNumber}` : ''}</p></div><span className="card-arrow">→</span></Link> : <div className="sports-empty"><strong>No active club membership.</strong><span>Historical memberships remain preserved below.</span></div>}</div>
      <div><div className="sports-section-head"><div><span className="eyebrow">CAREER STATS</span><h2>Official data only</h2></div></div><div className="stat-placeholder-grid premium-stat-grid"><div><strong>—</strong><span>Matches</span></div><div><strong>—</strong><span>Runs</span></div><div><strong>—</strong><span>Wickets</span></div><div><strong>—</strong><span>Rating</span></div></div><p className="data-note">Statistics activate after certified match data is connected. IPS will not invent or manually enter career totals.</p></div>
    </section>

    <section className="sports-section compact-section history-section"><div className="sports-section-head"><div><span className="eyebrow">TEAM HISTORY</span><h2>Membership timeline</h2></div></div>{player.memberships.length ? <div className="history-table premium-history-table"><div className="history-head"><span>Team / club</span><span>City</span><span>Period</span><span>Status</span></div>{player.memberships.map((membership) => <div className="history-row" key={membership.id}><span><strong>{membership.team?.name ?? 'Unknown team'}</strong><small>{membership.club?.name ?? 'Club'}</small></span><span>{membership.city?.name ?? '—'}</span><span>{formatDate(membership.start_on)} → {membership.end_on ? formatDate(membership.end_on) : 'Present'}</span><span className="history-status">{titleCase(membership.status)}</span></div>)}</div> : <div className="sports-empty"><strong>No membership history yet.</strong></div>}</section>
    <SiteFooter />
  </main>;
}
