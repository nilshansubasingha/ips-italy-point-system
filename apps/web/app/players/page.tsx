...
export default async function PlayersPage({searchParams}:{searchParams:Promise<{city?:string}>}){
 const {city}=await searchParams; const [players,cities]=await Promise.all([getPlayerDirectory(),getCities()]); const visible=city?players.filter(p=>p.city?.code.toLowerCase()===city.toLowerCase()):players; const attached=visible.filter(p=>p.currentTeam).length;
