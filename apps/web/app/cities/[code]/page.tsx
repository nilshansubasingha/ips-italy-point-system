...
import { getCityByCode } from '@ips/data';
...
export default async function CityPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return <main className="shell sports-shell"><SiteHeader />
    <section className="city-hero premium-city-hero">
      <div className="city-hero-copy"><span className="eyebrow light">IPS CITY HUB</span><h1>{city.name}</h1><p>{city.region ?? 'Italy'} · Clubs, players, tournaments and fixtures connected through one city identity.</p><div className="city-signal"><span>●</span> NATIONAL IPS NETWORK</div></div>
      <div className="city-hero-metrics"><div><strong>{city.clubs.length}</strong><span>Clubs</span></div><div><strong>{city.players.length}</strong><span>Players</span></div><div><strong>{city.tournaments.length}</strong><span>Tournaments</span></div><div><strong>{city.fixtures.length}</strong><span>Fixtures</span></div></div>
    </section>

    <section className="sports-section compact-section city-content-grid">
      <div className="city-main-column">
        <div className="sports-section-head"><div><span className="eyebrow">CLUBS</span><h2>{city.name} club network</h2></div><Link href={`/clubs?city=${city.code.toLowerCase()}`}>All clubs →</Link></div>
        <div className="club-grid premium-club-grid">{city.clubs.map((club, index) => <Link className="club-card premium-club-card" href={`/clubs/${club.slug}`} key={club.id}><span className="card-index">{String(index+1).padStart(2,'0')}</span><Crest name={club.name} imageUrl={club.logo_url} large/><div className="club-card-copy"><span>{club.verified ? 'Verified club' : 'Club'}</span><h3>{club.name}</h3><div className="club-card-stats"><b>{club.activePlayerCount}</b> players <b>{club.teams.length}</b> teams</div></div><span className="card-arrow">↗</span></Link>)}</div>
      </div>
      <aside className="context-rail city-context-rail"><div className="context-rail-card dark-rail"><span className="micro-label">CITY IDENTITY</span><h3>One local hub. National visibility.</h3><p>Every club, tournament and fixture in {city.name} remains connected to the Italy-wide IPS dataset.</p></div><div className="context-rail-card"><span className="micro-label">QUICK LINKS</span><div className="rail-link-list"><Link href={`/players?city=${city.code.toLowerCase()}`}>Players<span>→</span></Link><Link href={`/tournaments?city=${city.code.toLowerCase()}`}>Tournaments<span>→</span></Link><Link href="/rankings">Rankings<span>→</span></Link><Link href="/match-centre">Match Centre<span>→</span></Link></div></div></aside>
    </section>
