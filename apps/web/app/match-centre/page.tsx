import { getActiveCities, getFixtureContexts, getMatchLiveSummaries } from '@ips/data';
...
export default async function MatchCentrePage() {
  const [cities, fixtures, liveSummaries] = await Promise.all([getActiveCities(50), getFixtureContexts(), getMatchLiveSummaries()]);
  ...
  return <main className="shell sports-shell"><SiteHeader />
    ...
    <section className="sports-section no-top"><MatchCentre fixtures={fixtures} cities={cities} liveSummaries={liveSummaries}/></section>
    <SiteFooter />
  </main>;
}
