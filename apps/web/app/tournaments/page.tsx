...
import {getActiveCities,getTournamentDirectory} from '@ips/data';
...
export default async function TournamentsPage({searchParams}:{searchParams:Promise<{city?:string}>}){
 const {city}=await searchParams; const [tournaments,cities]=await Promise.all([getTournamentDirectory(),getActiveCities(50)]); const visible=city?tournaments.filter(t=>t.city?.code.toLowerCase()===city.toLowerCase()):tournaments; const featured=visible[0]??null;
 return <main className="shell sports-shell"><SiteHeader/>
  ...
  <section className="directory-toolbar-wide"><ActiveCityFilter cities={cities} basePath="/tournaments" selectedCode={city}/><div className="directory-count"><strong>{visible.length}</strong> tournaments</div></section>
