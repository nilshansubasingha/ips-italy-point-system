...
export default async function MatchControllerPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  const {id:identifier}=await params; const sp=await searchParams;
  ...
  if(!user) return <main className="controller-portal"><div className="portal-card"><h1>Sign in required.</h1><p>Use your IPS account before opening the Match Controller.</p><Link className="portal-button" href={`/auth/login?next=/matches/${encodeURIComponent(identifier)}`}>Sign in to Controller →</Link></div></main>;
  ...
  const {data:matchRef,error:matchRefError}=await matchLookup;
  if(matchRefError||!matchRef)notFound();
