...
export default async function ControllerHome() {
  ...
  if (!user) return <main className="controller-portal"><div className="portal-card"><BrandMark/><span className="micro">IPS MATCH CONTROLLER</span><h1>Sign in first.</h1><p>The Controller uses the same IPS account and scoped permissions as the website.</p><Link className="portal-button" href={`${WEB_URL}/auth/login`}>Sign in on IPS →</Link></div></main>;
  ...
  return <main className="controller-portal">
    ...
    <section className="controller-match-list">
      ...
      {!matches.length && <div className="portal-empty"><strong>No controller matches available.</strong><p>Assign this account as a scorer, or open the Controller as a tournament administrator/Owner.</p><Link href={`${WEB_URL}/manage/tournaments`}>Tournament operations →</Link></div>}
    </section>
  </main>;
}
